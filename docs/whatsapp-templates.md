...
the website already sends at the moment of registration via
`POST /api/notifications/whatsapp/send` — that instant send is the registrant's
confirmation, alongside the website's email confirmation (`Pratipal Website/src/app/api/invitations/route.ts`),
